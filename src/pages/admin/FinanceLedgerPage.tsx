...
export function FinanceLedgerPage() {
    ...
    const currentCycleEntries = useMemo(() => {
        return ledgerEntries.filter(e => e.payrollCycleId === payrollStatus.id);
    }, [ledgerEntries, payrollStatus.id]);

    const pendingFunding = currentCycleEntries.filter(e => e.status === 'Pending Funding');
    const fundedReady = currentCycleEntries.filter(e => e.status === 'Funded');
    const executed = currentCycleEntries.filter(e => e.status === 'Executed');

    // Totals
    const pendingTotal = pendingFunding.reduce((sum, e) => sum + e.amount, 0);
    const fundedTotal = fundedReady.reduce((sum, e) => sum + e.amount, 0);

    const handleFundingApproval = (pin: string) => {
        // @ts-ignore
        const result = (approveLedgerFunding as any)(payrollStatus.id, pin);
        if (result.success) {
            showSuccess('Funding Approved', 'Ledger is now ready for execution.');
            setActiveTab('ready'); // Auto-switch
        } else {
            showError('Approval Failed', result.error || 'Invalid PIN');
        }
    };

    const handleExecution = () => {
        // @ts-ignore
        const result = (executeLedgerBatch as any)(payrollStatus.id);
        if (result.success) {
            showSuccess('Payments Executed', `Successfully processed ${fundedReady.length} payments.`);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-slate-900">₦{ledgerEntries.filter(e => e.status === 'Executed').reduce((s, e) => s + e.amount, 0).toLocaleString()}</div>
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {(activeTab === 'pending' ? pendingFunding : activeTab === 'ready' ? fundedReady : executed).length === 0 ? (
                                    ...
                                    (activeTab === 'pending' ? pendingFunding : activeTab === 'ready' ? fundedReady : executed).map(entry => (
            ...
            {/* Initial Generation Trigger (Testing/Mock) */}
            {/* In real flow, this is triggered by Payroll Close. Keeping hidden or restricted? */}
            {/* 
            <div className="p-4 bg-slate-100 rounded text-xs text-slate-500">
                Debug: <button onClick={() => generateLedger(payrollStatus.id)} className="underline">Force Generate Ledger for Current Cycle</button>
            </div> 
            */}
