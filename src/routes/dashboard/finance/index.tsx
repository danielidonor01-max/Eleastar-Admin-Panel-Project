...
export function FinanceLedgerPage() {
    ...
    const approveLedgerFunding = useFinanceStore((s) => s.approveLedgerFunding);
    const executeLedgerBatch = useFinanceStore((s) => s.executeLedgerBatch);
    ...
    const handleFundingApproval = async (pin: string) => {
        ...
        if (result.success) {
            toast.success('Funding Approved', { description: 'Ledger is now ready for execution.' });
            setActiveTab('ready');
        } else {
            toast.error('Approval Failed', { description: result.error || 'Invalid PIN' });
        }
    };
    ...
    const handleExecution = async () => {
        ...
        if (result.success) {
            toast.success('Payments Executed', { description: `Successfully processed ${fundedReady.length} payments.` });
            setActiveTab('history');
        } else {
            toast.error('Execution Failed', { description: result.error || 'Unknown error' });
        }
    };
