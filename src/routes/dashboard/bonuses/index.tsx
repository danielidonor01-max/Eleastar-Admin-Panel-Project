...
import type { BonusType, BonusRequest, Employee } from '@/types';
import { BonusApprovalModal } from '@/components/BonusApprovalModal';

export function BonusPage() {
    const employees = useEmployeeStore((s) => s.employees);
    const bonusTypes = useBonusStore((s) => s.bonusTypes);
    const bonusRequests = useBonusStore((s) => s.bonusRequests);
    const createBonusType = useBonusStore((s) => s.createBonusType);
    const updateBonusType = useBonusStore((s) => s.updateBonusType);
    const requestBonus = useBonusStore((s) => s.requestBonus);
    const approveBonus = useBonusStore((s) => s.approveBonus);
    const rejectBonus = useBonusStore((s) => s.rejectBonus);
                        ...
                        <form onSubmit={handleSaveType} className="space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => setTypeForm({ ...typeForm, category: e.target.value as BonusType['category'] })}
                                    className="w-full p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500"
                                >
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex gap-4 items-center">
                            ...
                            >
                                ...
                                {bonusTypes.map((t: BonusType) => <option key={t.id} value={t.id}>{t.name}</option>)}
                            </select>
                        </div>
