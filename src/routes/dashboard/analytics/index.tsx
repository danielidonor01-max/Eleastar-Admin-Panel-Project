...
import type { Employee, PerformanceReview, BonusRequest, PromotionRequest, LeaveRequest } from '@/types';
import {
    Users,
    TrendingUp,
    DollarSign,
...
export function AnalyticsDashboard() {
    ...
    const bonusRequests = useBonusStore((s) => s.bonusRequests);
    const promotionRequests = usePromotionStore((s) => s.promotionRequests);
    const currentUserRole = useAuthStore((s) => s.currentUserRole);
    const refreshPromotions = usePromotionStore((s) => s.refreshPromotions);
    const refreshBonuses = useBonusStore((s) => s.refreshBonuses);
    const refreshLeaveRequests = useLeaveStore((s) => s.refreshLeaveRequests);
    ...
    const healthMetrics = useMemo(() => {
        ...
        const suspended = employees.filter((e: Employee) => e.status === 'suspended').length;
        const exited = employees.filter((e: Employee) => e.status === 'exited').length;

        const ratings = performanceReviews
            .filter((r: PerformanceReview) => r.status === 'Approved')
            .map((r: PerformanceReview) => r.rating || r.managerRating || 0);

        const ratingDist = [
            { name: '0-1 Stars', count: ratings.filter((r: number) => r >= 0 && r < 1).length },
            { name: '1-2 Stars', count: ratings.filter((r: number) => r >= 1 && r < 2).length },
            { name: '2-3 Stars', count: ratings.filter((r: number) => r >= 2 && r < 3).length },
            { name: '3-4 Stars', count: ratings.filter((r: number) => r >= 3 && r < 4).length },
            { name: '4-5 Stars', count: ratings.filter((r: number) => r >= 4 && r <= 5).length },
        ];
    ...
    const costMetrics = useMemo(() => {
        ...
        const totalBonusesApproved = bonusRequests
            ...
            .reduce((sum: number, r: BonusRequest) => sum + r.amount, 0);
    ...
    const riskMetrics = useMemo(() => {
        const pendingPromotions = promotionRequests.filter((r: PromotionRequest) => r.status === 'Pending').length;
