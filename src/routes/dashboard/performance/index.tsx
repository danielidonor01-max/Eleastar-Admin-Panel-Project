...
import type { Employee, PerformanceReview, Task, ReviewCycle, TaskPerformanceStats } from '@/types';
...
export const PerformancePage = () => {
    ...
    const createReviewCycle = usePerformanceStore((s) => s.createReviewCycle);
    const updatePerformanceReview = usePerformanceStore((s) => s.updatePerformanceReview);
    const startReviewCycle = usePerformanceStore((s) => s.startReviewCycle);
    const submitSelfReview = usePerformanceStore((s) => s.submitSelfReview);
    const approvePerformanceReview = usePerformanceStore((s) => s.approvePerformanceReview);
    const requestRevision = usePerformanceStore((s) => s.requestRevision);
    const refreshReviewCycles = usePerformanceStore((s) => s.refreshReviewCycles);
    ...
    const handleApprove = () => {
        ...
        if (!reviewData.managerFeedback.trim()) {
            toast.error('Validation Error', { description: 'Feedback is required before approving.' });
            return;
        }
    ...
    const handleRequestRevision = () => {
        if (!selectedReviewId) return;
        if (!reviewData.managerFeedback.trim()) {
            toast.error('Validation Error', { description: 'Please provide feedback explaining what needs revision.' });
            return;
        }
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {(() => {
                                ...
                                return stats.map((stat: TaskPerformanceStats) => (
                                    <tr key={stat.id} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-brand-50 text-brand-600  items-center justify-center font-bold text-xs border border-brand-100 hidden sm:flex">
                                                    {stat.name.charAt(0)}
                                                </div>
                                                <div>
            ...
            <div className="grid grid-cols-1 gap-6">
                {reviewCycles.map((cycle: ReviewCycle) => {
                    const cycleReviews = performanceReviews.filter((r: PerformanceReview) => r.id === cycle.id);
                            ...
                            <div className="bg-slate-50 p-4">
                                ...
                                {completionCount === 0 ? (
                                    ...
                                    <div className="space-y-2">
                                        {cycleReviews.map((review: PerformanceReview) => (
                    ...
                    <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95">
                        ...
                        <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                            <div>
                                ...
                                <div className="text-sm text-slate-500">
                                    {getEmployeeName(performanceReviews.find((r: PerformanceReview) => r.id === selectedReviewId)?.employeeId || '')}
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                ...
                                {(() => {
                                    const review = performanceReviews.find((r: PerformanceReview) => r.id === selectedReviewId);
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={e => setReviewData({ ...reviewData, recommendation: e.target.value as 'None' | 'Promotion' | 'Salary Increase' | 'Bonus' })}
                                    >
                                ...
                                <div>
                                    ...
                                    <textarea
                                        id="internalNotes"
                                        rows={2}
                                        className="w-full px-3 py-2  rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none bg-amber-50 border-amber-200"
